...
def translate_file(model,
  ...
  def predict_step(inputs):
    ...
    def _step_fn(inputs):
      ...
      tag = inputs[0]
      val_inputs = inputs[1]
      val_outputs, decoded_scores = model([val_inputs], mode="predict", training=False)
      return tag, val_outputs, decoded_scores
  ...
  # if not params["use_tpu"] and not params["use_ctl"]:
  #   ## CPU model creation
  #   model = models.create_model("transformer", params)
  #   new_model_flag = True


  ## CPU weight load
  # for latest_checkpoint in tf.train.checkpoints_iterator(model_dir):
  #   if new_model_flag:
  #     checkpoint = tf.train.Checkpoint(model=model)
  #     checkpoint.restore(latest_checkpoint).expect_partial()
  #     logging.info("Loaded checkpoint %s", latest_checkpoint)

  ## enable this line for TPU
  if True:
    for i, text in enumerate(input_generator()):
      if distribution_strategy:
        ...
        outputs = distribution_strategy.experimental_local_results(
            predict_step(text))
        tags, unordered_val_outputs, unordered_decoded_scores = outputs[0]
        ...
        unordered_decoded_scores = [
            decoded_score.numpy() for decoded_score in unordered_decoded_scores._values]
        # pylint: enable=protected-access
        val_outputs = [None] * len(tags)
        decoded_scores = [None] * len(tags)
        for k in range(len(tags)):
          val_outputs[tags[k]] = unordered_val_outputs[k]
          decoded_scores[tags[k]] = unordered_decoded_scores[k]
        val_outputs = np.reshape(val_outputs, [params["decode_batch_size"], -1])
      else:
        val_outputs, decoded_scores = model([text], mode="predict")
        # print ('val_outputs', val_outputs)
        # print ('decoded_scores', decoded_scores)
      ...
      for j in range(length):
        if j + i * batch_size < total_samples:
          if i < 10 and j == 0:
            print ('i', i, 'j', j)
            print ('val_outputs[j]', val_outputs[j])
            print ('decoded_scores[j]', decoded_scores[j])
