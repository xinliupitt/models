...
def continuous_eval(strategy,
  ...
  ds = data_pipeline.train_input_fn(params)
  ds = ds.map(lambda x, y: x).take(_SINGLE_SAMPLE)
  test_dataset = ds
  print ('test_dataset', test_dataset)
  # iter1 = iter(test_dataset)
  # print ('next iter', next(iter1))
  ...
  def test_step(inputs):
    ...
    def _test_step_fn(inputs):
      ...
      # logits, _, _ = model(inputs, mode="train")
      # metric_layer([logits, inputs[1]])

      # Get logits from top beam search results for bleu and rouge metrics.
      logits = model([inputs], mode="predict")
